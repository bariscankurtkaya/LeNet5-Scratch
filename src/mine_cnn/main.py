...
import numpy as np
from network import convolution, max_pool, softmax
...
conv_bias2 = np.random.rand(6, 1)

fc_weight1 = np.random.rand(size = (120, 400))
fc_bias1 = np.random.rand(120)

fc_weight2 = np.random.rand(size = (84, 120))
...
for i in range(len(train_input)):
    current_image = train_input[1][0].shape
    padded_image = np.pad(current_image, 2, mode='constant')
    ...
    conv_output2 = convolution(max_pool_output1, conv_kernel1, conv_bias2)
